...
if (1) {
  ...
  document.querySelectorAll('div.editable').forEach(editable => {
    ...
    editable.onkeydown = function keydownHandler(keyboardEvent) {

      // keyboardEvent fires before inputEvent -> detect forward delete
      //if (keyboardEvent.keyCode == 46) { // forward delete
      if (keyboardEvent.keyCode == 8) { // backward delete
        window.selection.isForwardDelete = false;
      }
      else {
        window.selection.isForwardDelete = true; // default is forward delete
      }
      //if (keyboardEvent.keyCode == 8) // backward delete
    ...
    editable.oninput = function(inputEvent) {
      ...
      if (selection.baseNode.parentNode.localName == 'ins') {
        ...
        if (selection.baseNode.parentNode.data == '') {
          console.log('remove empty <ins> node', selection.baseNode.parentNode);
      ...
      if (inputEvent.inputType == 'insertText') {
        ...
        console.log(`insert: data = ${JSON.stringify(inputEvent.data)}`)
        console.log('selection', selection);
        if (selection.baseNode.nodeType == 3) {
          // text node
          //if (selection.baseNode.parentNode.localName == 'ins') return; // noop
          // start new <ins> tag
          console.log('start new <ins> tag in baseNode', selection.baseNode);
      ...
      */
      ...
      if (ws.baseNode == ws.extentNode) {
        ...
        if (inputEvent.inputType[0] == 'd') { // deleteXXX
          ...
          console.log(`ws.baseNodeClone`, ws.baseNodeClone); // before change
          console.log(`ws.baseNode     `, ws.baseNode); // after change
          ...
          if (baseNodeCloneData.length < cuta2) { // cuta2 is out of bound
            ...
            var e = document.createTextNode(h.slice(cutb1));
              nodesFragment.appendChild(e);
            console.log(`nodesFragment: <fragment>${nodesFragmentDebugArray.join('')}</fragment>`);
